        ...
        if lemma not in nouns:
            ...
            if len(word) > 1 and word in nouns:
                for key in nouns[word]:
                    if key == "gender":
                        nouns[lemma][key] = nouns[word][key]
                    else:
                        try:
                            nouns[lemma][key] = rest.capitalize()+nouns[word][key].lower()
                        except:
                            nouns[lemma][key] = lemma
            # Default
            else:
                # Genus
                # Maskulin
                if "gender" not in nouns[lemma]:
                    
                    for suffix in ['ling', 'ich', 'us', 'er', 'ig']:
                        if suffix in lemma and lemma[-len(suffix):] == suffix:
                            nouns[lemma]["gender"] = "m"
                # Feminin
                if "gender" not in nouns[lemma]:
                    for suffix in ['schaft', 'keit', 'heit', 'enz', 'anz', 'tät', 'ion', 'ung', 'ie', 'ik']:
                        if suffix in lemma and lemma[-len(suffix):] == suffix:
                            nouns[lemma]["gender"] = "f"
                # Neutral
                if "gender" not in nouns[lemma]:
                    for suffix in ['ment', 'chen', 'lein', 'ing', 'en', 'ma', 'um']:
                        if suffix in lemma and lemma[-len(suffix):] == suffix:
                            nouns[lemma]["gender"] = "n"
                # Default: Random
                if "gender" not in nouns[lemma]:
                    nouns[lemma]["gender"] = choice(["n","f","m"])
                
                # Genitiv Singular
                if "gen_sg" not in nouns[lemma]:
                    # n-Deklination
                    if lemma[-1] == "e":
                        nouns[lemma]["gen_sg"] = lemma+"n"
                        nouns[lemma]["dat_sg"] = lemma+"n"
                        nouns[lemma]["weak"] = True
                    else:
                        nouns[lemma]["gen_sg"] = lemma+"s"
                # Nominativ Plural
                # Endung -n/-en
                if "nom_pl" not in nouns[lemma]:
                    for suffix in ['schaft', 'keit', 'heit', 'ung', 'tät', 'ion', 'ist', 
                                   'ant', 'and', 'ent', 'us', 'um', 'ma', 'ik', 'in', 'or', 'e']:
                        if suffix in lemma and lemma[-len(suffix):] == suffix:
                            if lemma[-1] in ["a","e"]:
                                nouns[lemma]["nom_pl"] = lemma[:-1]+"en"
                            elif lemma[-2] == "in":
                                nouns[lemma]["nom_pl"] = lemma[:-1]+"nen"
                            else:
                                nouns[lemma]["nom_pl"] = lemma+"en"
                if "nom_pl" not in nouns[lemma]:
                    # Endung -e
                    for suffix in ['ling', 'ier', 'ich', 'eur', 'ör', 'ig']:
                        if suffix in lemma and lemma[-len(suffix):] == suffix:
                            nouns[lemma]["nom_pl"] = lemma+"e"
                            nouns[lemma]["dat_pl"] = lemma+"en"
                if "nom_pl" not in nouns[lemma]:
                    # Endung -s
                    for suffix in ['y', 'u', 'o', 'i', 'a']:
                        if suffix in lemma and lemma[-len(suffix):] == suffix:
                            nouns[lemma]["nom_pl"] = lemma+"s"
                if "nom_pl" not in nouns[lemma]:
                    # keine Endung
                    nouns[lemma]["nom_pl"] = lemma
...
class Verb():
    def __init__(self,lemma):
        ...
        if self.lemma not in verbs or self.lemma in verbs and None in [verbs[self.lemma][i] for i in verbs[self.lemma]]:
            ...
            for prefix in ['zu', 'da', 'an', 'ab', 'her', 'weg', 'ein', 'rum', 'aus', 'mit', 'auf', 'bei', 'hin', 'vor', 'los', 'fest', 'hoch', 'fort', 'fern', 'fehl', 'nach', 'heim', 'hinzu', 'neben', 'voran', 'herum', 'herzu', 'heran', 'hinan', 'empor', 'vorab', 'hinab', 'weiter', 'hinweg', 'vorweg', 'vorher', 'herein', 'zurück', 'vorauf', 'herbei', 'vorbei', 'hinauf', 'hervor', 'heraus', 'hinaus', 'nieder', 'hinein', 'herauf', 'voraus', 'entzwei', 'entlang', 'herüber', 'zurecht', 'hinüber', 'vorüber', 'herherab', 'entgegen', 'hinunter', 'herunter', 'zusammen', 'hernieder', 'hinterher', 'gegenüber', 'auseinander']:
                if len(self.lemma) >= len(prefix) and prefix == self.lemma[:len(prefix)]:
                    seperable_prefix = prefix
            # Stamm
            unkSTEM = self.stem
            # 1P SG Stamm
            if self.lemma[-3:] == "eln":
                stem_1p = self.lemma[:-3]+"le"
            else:
                stem_1p = self.stem
            # Suffixe bestimmen
            try:
            ...
            if self.lemma in verbs and None in [verbs[self.lemma][i] for i in verbs[self.lemma]]:
                for key in verbs[self.lemma]:
                    if verbs[self.lemma][key] != None:
                        default_entry[key] = verbs[self.lemma][key]
                
            verbs[self.lemma] = default_entry
